...
exports.capturePayment = async (req, res) => {
    ...
    for (const course_id of courses) {
        ...
        try {
            ...
            if (course.studentsEnrolled && course.studentsEnrolled.includes(userId)) {
...
const enrolledStudents = async (courses, userId, res) => {
    ...
    for (const courseID of courses) {
        ...
        if (!enrolledCourse.studentsEnrolled) enrolledCourse.studentsEnrolled = [];
        enrolledCourse.studentsEnrolled.push(userId);
        await Course.findByIdAndUpdate(courseID, enrolledCourse, { new: true });
        ...
        if (!enrolledStudent.courses) enrolledStudent.courses = [];
        if (!enrolledStudent.courseProgress) enrolledStudent.courseProgress = [];
        enrolledStudent.courses.push(courseID);
        enrolledStudent.courseProgress.push(courseProgress._id);
        const updatedStudent = await User.findByIdAndUpdate(userId, enrolledStudent, {new: true});
        // enrolled student ko mail send kardo
        const emailResponse=await mailSender(
            updatedStudent.email,
            `successfully Enrolled into ${enrolledCourse.courseName}`,
            courseEnrollmentEmail(enrolledCourse.courseName,`${updatedStudent.firstName}`+ " "+`${updatedStudent.lastName}` )
            )
