...
exports.resetPasswordToken = async (req, res) => {
	...
	try {
		...
		userToUpdate.resetPasswordExpires = Date.now() + 3600000;
		const updatedDetails = await User.findByIdAndUpdate(userToUpdate._id, userToUpdate, { new: true });
...
exports.resetPassword = async (req, res) => {
	try {
		...
		const userToReset = await User.findOne({ token: token });
		userToReset.password = encryptedPassword;
		await User.findByIdAndUpdate(userToReset._id, userToReset, { new: true });
		res.json({
			success: true,
